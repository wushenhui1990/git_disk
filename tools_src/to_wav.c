...
static int p_data(char *file_name, int rate_times, int base_rate)
{
    int read_len;
    int data_len = 0;
    short bytes;
    ...
    while(fscanf(fin, "%X %X %X %X %X %X %X %X %X %X %X %X %X %X %X %X\n", 
    ...
    if(base_rate == 44100){
        set_wav_header(2, (44100 * rate_times),  w_data_size);
    }else{
        set_wav_header(2, (48000 * rate_times),  w_data_size);
    }
...
{
    if(argc == 3){
        return p_data(argv[1], atoi(argv[2]), 44100);
    }else if (argc == 4){
        return p_data(argv[1], atoi(argv[2]), atoi(argv[3]));
    }else{
        printf("args: txt, rate_times, [base rate]\n");
    }
}
